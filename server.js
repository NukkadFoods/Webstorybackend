...
// 📊 Redis Load Balancer Status Endpoint
    ...
    res.json({
      ...
      loadBalancer: {
        ...
        connections: stats.connections.map(c => ({
          ...
          errors: c.errors,
          healthy: c.healthy,
          usagePercent: stats.totalRequests > 0
            ? ((c.requests / stats.totalRequests) * 100).toFixed(1) + '%'
            : '0%'
        }))
      },
      message: `Load balanced across ${stats.totalConnections} Redis instance(s)`,
      timestamp: new Date()
    });
  } catch (error) {
    res.status(500).json({
...
app.get('/api/cache-status', async (req, res) => {
  try {
    ...
    const redisLoadBalancer = require('./config/redisLoadBalancer');
    const redisLBStats = redisLoadBalancer.getStats();

    res.json({
      success: true,
      redis: redisStatus,
      redisLoadBalancer: redisLBStats,
      groq: groqStats,
      architecture: {
        browser: '5 minutes TTL',
        redis: '30 minutes TTL (load balanced)',
        mongodb: 'Source of truth',
        refreshStrategy: 'Lazy refresh (on-access)',
        note: 'Redis requests distributed across multiple Upstash instances'
      },
      timestamp: new Date()
    });
  } catch (error) {
